...
test=daisy(merge2[,c(2:4)],metric = "euclidean",
               stand=TRUE)
```

```{r}
testcluster=hclust(test,method='complete')
```


```{r}
plot(testcluster,cex=0.5)
```

```{r}
plot(testcluster,cex=0.4)
rect.hclust(testcluster, k = 3)
```

```{r}
demsil3=silhouette(cutree(testcluster, k = 3), test)
...
```{r}
solution$region=row.names(clust3solution)
row.names(solution)=NULL
```

```{r}
merge3=merge(merge2, solution)
```
